...
def execute_helper(dataset, algorithm, corr_threshold, const_threshold, seed, line_number):
    try:
        ...
        print(f"Erro capturado: {str(e)} para {dataset}, {algorithm}, {corr_threshold}, {const_threshold}, {seed}")
        return None, error

def main():
    # Removendo a verificação if __name__ == '__main__' para garantir que o código seja executado
    directory = '../datasets'
    ...
    # Garantir que o arquivo errors.csv seja criado mesmo se não houver erros
    ...
    if errors:
        ...
        df_errors = pd.DataFrame(columns=['error', 'dataset', 'algorithm', 'corr_threshold', 'const_threshold', 'seed', 'line_number'])
    
    df_errors.to_csv('errors/errors.csv', index=False)
    print(f"errors.csv has been saved with {len(errors)} errors")

# Chamada direta da função main para garantir execução
main()
